...
class Trainer(object):
    ...
    def __init__(self, cfg: FairseqConfig, task, model, criterion, quantizer=None):
        ...
        for param in self._model.classification_heads.parameters():
    ...
    def cyclic_adjust_precision(self, _iter, cyclic_period=CYCLIC_PERIOD_N):
        
        if CYCLIC:
            assert len(CYCLIC_BITS_SCHEDULE) == 2
    
            num_bit_min = CYCLIC_BITS_SCHEDULE[0]
            num_bit_max = CYCLIC_BITS_SCHEDULE[1]
            ...
            if _iter % LOG_FREQ == 0:
                logging.info('Iter [{}] num_bits = {} cyclic precision'.format(_iter, num_bits))
    ...
    def train_step(self, samples, curr_epoch=0, raise_oom=False):
        """Do forward, backward and parameter update."""
        self._set_seed()
        self.model.train()
        self.criterion.train()
        self.zero_grad()
        self.cyclic_adjust_precision(_iter=curr_epoch)
